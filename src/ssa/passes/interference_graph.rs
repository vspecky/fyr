...
use crate::{
    ssa::{function::FunctionData, passes, Value},
    utils::{EntityId, UxoResult},
};
...
impl InterferenceGraph {
    ...
    fn construct_from_function(
        func: &FunctionData,
        liveness: &passes::LivenessAnalysis,
    ) -> UxoResult<Self, InterferenceGraphError> {
...
impl passes::Pass for InterferenceGraph {
    ...
    fn dependencies(mut tracker: passes::PassDepTracker) -> UxoResult<(), Self::Error> {
        tracker
            .dependency::<passes::LivenessAnalysis>()
    ...
    fn execute(store: passes::PassStore) -> UxoResult<Self, Self::Error> {
        let liveness = store
            .get_pass::<passes::LivenessAnalysis>()
            .change_context(InterferenceGraphError::DependentPassFailed)?;
        let liveness = liveness.borrow();

        InterferenceGraph::construct_from_function(store.get_func(), &liveness)
    }
}
...
mod tests {
    ...
    fn test_interference_graph_calculation() {
        ...
        let interf = test_utils::get_pass::<InterferenceGraph>(&mut func);
        let interf = interf.borrow();
        println!();
