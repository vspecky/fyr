...
pub mod test_utils {
    ...
    pub fn get_function_with_single_loop() -> FunctionData {
        fyrc_ssa_builder::build_function! {
            ...
            (c67 = const32 67)
            (cr = icmp v, c67, Slt)
            (br cr, inside_while, continuation)
    ...
    pub fn get_function_with_nested_loops() -> FunctionData {
        fyrc_ssa_builder::build_function! {
            ...
            (c = const8 10)
            (cr = icmp v, c, Slt)
            (br cr, while_inside, continuation)
            ...
            (c = const8 30)
            (cr = icmp c, v, Slt)
            (br cr, n_inside_if_header, n_continuation)
            ...
            (c = const8 40)
            (cr = icmp c, v, Slt)
            (br cr, n_inside_if_then, n_inside_if_else)
